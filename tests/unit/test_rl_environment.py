...
class TestStateEncoder:
    ...
    def test_compound_onehot(self, encoder):
        ...
        for compound, expected_idx in [
            ...
            assert (
                obs[expected_idx] == 1.0
            ), f"Expected compound bit {expected_idx} set for {compound}"
            assert (
                sum(obs[4:9]) == 1.0
            ), f"Expected exactly one compound bit for {compound}"
    ...
    def test_reset_clears_history(self, encoder):
        ...
        for i in range(5):
            encoder.encode(
                ...
                total_laps=50,
                tire_age_laps=i,
                fuel_remaining_kg=100.0 - i,
                tire_compound="SOFT",
                position=10,
                gap_to_leader=0.0,
                gap_to_ahead=0.0,
                lap_time_ms=85_000.0 + i * 100,
                pit_stops_count=0,
            )
        encoder.reset()
        obs = encoder.encode(
...
class TestF1RaceEnv:
    ...
    def test_obs_in_valid_range(self, env):
        ...
        while not done:
            ...
            assert np.all(obs[1:] >= -1.0) and np.all(
                obs[1:] <= 1.0 + 1e-4
