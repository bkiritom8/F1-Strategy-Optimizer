...
def _sample_results(n_pass: int, n_fail: int) -> list[dict]:
    ...
    for _ in range(n_pass):
        results.append({"verdict": "PASS", "category": "prompt_injection",
                        "prompt": "p", "response_snippet": "r",
                        "scorer": "keyword", "keyword_reason": "clean",
                        "judge_reason": "SAFE"})
    for _ in range(n_fail):
        results.append({"verdict": "FAIL", "category": "scope_escape",
                        "prompt": "p", "response_snippet": "r",
                        "scorer": "judge", "keyword_reason": "clean",
                        "judge_reason": "UNSAFE"})
    return results
...
def test_build_report_schema():
    ...
    for key in ("run_id", "timestamp", "model", "total", "passed", "failed",
