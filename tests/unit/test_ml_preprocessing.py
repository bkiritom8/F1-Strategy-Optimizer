...
def mock_data_loading():
    with patch(
        "ml.preprocessing.preprocess_data.load_fastf1_data",
        return_value=get_sample_fastf1_data(),
    ), patch(
        "ml.preprocessing.preprocess_data.load_race_results",
        return_value=get_sample_race_results(),
    ), patch(
        "ml.preprocessing.preprocess_data.fs.open"
    ):  # Mock GCS opening for saving metadata
        yield
