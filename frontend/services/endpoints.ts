...
export async function fetchDrivers(): Promise<DriverProfile[]> {
  ...
  try {
    ...
    // Sort so modern drivers (in our map) come first, others after
    const sorted = [...staticDrivers].sort((a, b) => {
      const aKnown = DRIVER_TEAM_MAP[a.id] ? 1 : 0;
      const bKnown = DRIVER_TEAM_MAP[b.id] ? 1 : 0;
      return bKnown - aKnown;
    });

    // Take top 40 (modern + prominent historical) to avoid cluttering matrix with 800+ dots
    return sorted.slice(0, 40).map((d) => {
      const rng = seedRandom(`static_driver_${d.id}`);
      
      // Independent trait generation to avoid linear correlation
      const getScore = (offset: number) => Math.min(100, 70 + (rng() * 25) + offset);
      
      return {
        driver_id: d.id,
        name: d.name,
        team: DRIVER_TEAM_MAP[d.id] || 'Historical / Unknown',
        code: d.code || d.id.slice(0, 3).toUpperCase(),
        nationality: d.nationality || '',
        career_races: Math.floor(10 + rng() * 100),
        career_wins: Math.floor(rng() * 10),
        aggression_score: getScore(rng() * 5),
        consistency_score: getScore(rng() * -5),
        pressure_response: getScore(0),
        tire_management: getScore(rng() * 10),
        wet_weather_skill: getScore(rng() * 15),
        qualifying_pace: getScore(rng() * 8),
        race_pace: getScore(rng() * 12),
        overtaking_ability: getScore(rng() * 6),
        defensive_ability: getScore(rng() * 4),
        fuel_efficiency: getScore(rng() * 10),
