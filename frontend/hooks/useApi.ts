...
/**
 * Executes a full Monte Carlo strategy simulation on the backend.
 * 
 * @param params - Simulation inputs (race_id, driver_id, proposed strategy stints).
 */
export function useStrategySimulation(params: {
  race_id:   string;
  driver_id: string;
  ...
  return useApiCall<any>(
    async () => {
      if (!params) throw new Error('No simulation params provided');
      return simulateStrategy(params);
    },
    null,
    [params?.driver_id, JSON.stringify(params?.strategy), params?.regulation_set],
    `simulate(${params?.driver_id ?? 'none'}, ${params?.regulation_set ?? '2025'})`,
  );
}
