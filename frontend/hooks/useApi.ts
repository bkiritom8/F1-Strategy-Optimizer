...
import {
  ...
  fetchStaticRaces2025,
  fetchStaticRaces2026,
  fetchStaticSeasons,
...
function useApiCall<T>(
  ...
  const execute = useCallback(async () => {
    ...
    try {
      ...
      if (mountedRef.current) {
        setData(result);
        setIsLive(true);
        logger.debug(`[useApi] Live data received: ${label}`);
      }
    } catch (err: any) {
  ...
  useEffect(() => {
    ...
    logger.debug(`[useApi] Hook mounted: ${label}`);
    execute();
    return () => { 
      logger.debug(`[useApi] Hook unmounted: ${label}`);
...
/**
 * Fetches race results for a specific year.
 * Supports 1950–2026. Data extracted from the Ergast/OpenF1 pipeline.
 */
export function useRaces(year: number) {
  const fetcher = useCallback(() => {
    if (year === 2024) return fetchStaticRaces2024();
    if (year === 2025) return fetchStaticRaces2025();
    if (year === 2026) return fetchStaticRaces2026();
    // Support for other years via the generic static loader if backend doesn't exist
    return fetchStaticRaces2024(); // Fallback to 2024 for demo safety
  }, [year]);

  return useApiCall(fetcher, [], [year], `races(${year})`);
}

/**
 * @deprecated Use useRaces(2024) instead.
 */
export function useRaces2024() {
  return useRaces(2024);
}

/**
 * @deprecated Use useRaces(2025) instead.
 */
export function useRaces2025() {
  return useRaces(2025);
}

/**
 * @deprecated Use useRaces(2026) instead.
 */
export function useRaces2026() {
  return useRaces(2026);
}
