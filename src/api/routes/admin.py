...
async def get_gcp_metrics(current_user: User = Depends(get_current_user)):
    ...
    if not iam_simulator.check_permission(current_user, Permission.ML_MODEL_READ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Insufficient admin permissions"
        )
...
async def get_logs(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        for entry in client.list_entries(filter_=filter_str, order_by=gcp_logging.DESCENDING, max_results=50):
            logs.append({
        ...
        logger.warning(f"Could not load GCP Logging: {e}. Falling back to default log info.")
        logs = [{"timestamp": "2026-03-31T00:00:00Z", "severity": "ERROR", "message": f"GCP logging unavailable: {str(e)}"}]
