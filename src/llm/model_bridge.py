...
def _load(name: str) -> Any | None:
    ...
    try:
        buf = io.BytesIO()
        storage.Client(project="f1optimizer").bucket(_BUCKET).blob(_PATHS[name]).download_to_file(buf)
        buf.seek(0)
...
def _build_df(inputs: dict) -> pd.DataFrame:
    ...
    return pd.DataFrame([{
        ...
        "raceName": circuit, "constructor": "unknown",
        "LapNumber": lap, "total_laps": total,
        "TyreLife": tire_age, "Stint": stint,
        "Compound": compound,
        ...
        "mean_throttle": 75.0, "mean_brake": 20.0,
        "tyre_delta": 0.0, "deg_rate_roll3": 0.05,
        "position": position, "grid": position,
        "gap_ahead": gap, "speed_diff": 5.0,
        "drs_available": 1, "tyre_advantage": 0.0,
        "overtake_attempt": 0, "driving_style": "NEUTRAL",
...
def get_predictions(race_inputs: dict) -> dict[str, str]:
    ...
    if bundle:
        try:
            ...
            w = bundle["weight"]
            val = float(w * bundle["lgb"].predict(X)[0] + (1 - w) * bundle["xgb"].predict(X)[0])
