...
circuit_sc = (
    ...
    .agg(total_laps=('LapNumber', 'max'), sc_laps=('is_sc_lap', 'sum'), raceName=('raceName', 'first'))
    .reset_index()
)
circuit_sc['sc_prob_per_lap'] = circuit_sc['sc_laps'] / circuit_sc['total_laps']
circuit_prob = circuit_sc.groupby('raceName')['sc_prob_per_lap'].mean().round(4).to_dict()
...
with aiplatform.start_run(run='safety-car-v1'):
    ...
    for w in np.arange(0.1, 0.95, 0.05):
        pred = ((w * lgb_pit.predict_proba(X_val)[:, 1] +
                 (1 - w) * xgb_pit.predict_proba(X_val)[:, 1]) >= 0.5).astype(int)
    ...
    val_pred  = ((best_w * lgb_pit.predict_proba(X_val)[:, 1] +
                  (1 - best_w) * xgb_pit.predict_proba(X_val)[:, 1]) >= 0.5).astype(int)
    test_pred = ((best_w * lgb_pit.predict_proba(X_test)[:, 1] +
                  (1 - best_w) * xgb_pit.predict_proba(X_test)[:, 1]) >= 0.5).astype(int)
    ...
    aiplatform.log_metrics({
        ...
        'lgb_val_f1': lgb_val_f1, 'xgb_val_f1': xgb_val_f1,
    ...
    ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=['Stay Out', 'Pit']).plot(
    ...
    ax.bar(['LGB', 'XGB', 'Ensemble'], [lgb_val_f1, xgb_val_f1, val_f1],
    ...
    for season in sorted(val['season'].unique()):
        ...
        f1   = f1_score(y_val_series[mask], val_pred_series[mask], average='macro', zero_division=0)
    ...
    if f1_gap > 0.08:
        val_proba  = best_w * lgb_pit.predict_proba(X_val)[:, 1] + \
        ...
        if len(street_idx) > 10:
            ...
            for thresh in np.arange(0.3, 0.7, 0.05):
                ...
                f1_s   = f1_score(y_val_series[street_idx], pred_s, average='macro', zero_division=0)
                if f1_s > best_st_f1:
                    best_st_f1, best_st_thresh = f1_s, thresh
            print(f'  Street-adjusted threshold: {best_st_thresh:.2f}  (F1={best_st_f1:.3f})')
            aiplatform.log_metrics({'bias_mitigation_applied': 1,
        ...
        aiplatform.log_metrics({'bias_mitigation_applied': 0, 'bias_street_permanent_f1_gap': float(f1_gap)})
    ...
    for mcs in [10, 20, 40, 80]:
        m = LGBMClassifier(n_estimators=300, max_depth=7, min_child_samples=mcs,
    ...
    for nl in [15, 31, 63, 127]:
        m = LGBMClassifier(n_estimators=300, max_depth=-1, num_leaves=nl,
                           learning_rate=0.01, random_state=42, n_jobs=-1,
                           verbose=-1, class_weight='balanced')
        m.fit(X_train, y_train)
        f1 = f1_score(y_val, m.predict(X_val), average='macro')
    ...
    axes[0].axvline(x=LGB_PARAMS['min_child_samples'], color='red', linestyle='--',
