...
with aiplatform.start_run(run='pit-window-v1'):
    ...
    xgb_val_mae = float(mean_absolute_error(y_val_orig, np.expm1(xgb_model.predict(X_val_raw))))
    lgb_val_mae = float(mean_absolute_error(y_val_orig, np.expm1(lgb_model.predict(X_val_raw))))

    print('\nPIT WINDOW RESULTS')
    print(f'Val  — MAE: {val_mae:.3f} laps, R2: {val_r2:.3f}')
    print(f'Test — MAE: {test_mae:.3f} laps, R2: {test_r2:.3f}')
    print('\nIndividual models:')
    print(f'  XGB — Val MAE: {xgb_val_mae:.3f}, Test MAE: {mean_absolute_error(y_test_orig, np.expm1(xgb_model.predict(X_test_raw))):.3f}')
    print(f'  LGB — Val MAE: {lgb_val_mae:.3f}, Test MAE: {mean_absolute_error(y_test_orig, np.expm1(lgb_model.predict(X_test_raw))):.3f}')

    aiplatform.log_metrics({
        'val_mae': val_mae, 'val_r2': val_r2,
        'test_mae': test_mae, 'test_r2': test_r2,
        'xgb_val_mae': xgb_val_mae, 'lgb_val_mae': lgb_val_mae,
        'ensemble_weight_xgb': best_w,
    })

    # Model comparison bar chart
    fig, ax = plt.subplots(figsize=(6, 4))
    ax.bar(['XGB', 'LGB', 'Ensemble'], [xgb_val_mae, lgb_val_mae, val_mae],
    ...
    sample  = np.random.choice(len(y_val_orig), size=min(3000, len(y_val_orig)), replace=False)
    ax.scatter(y_val_orig.iloc[sample], val_pred[sample], alpha=0.2, s=5, color='steelblue')
    ...
    ax.set_title(f'Pit Window — Predicted vs Actual\nVal MAE={val_mae:.3f} laps  R2={val_r2:.3f}')
    ax.legend()
    ...
    if mae_gap > 1.5:
        ...
        aiplatform.log_metrics({'bias_mitigation_applied': 1, 'bias_soft_hard_mae_gap': float(mae_gap)})
    else:
        print('  Gap within tolerance — no reweighting required')
        aiplatform.log_metrics({'bias_mitigation_applied': 0, 'bias_soft_hard_mae_gap': float(mae_gap)})

    # SHAP sensitivity analysis
    print('\nComputing SHAP values...')
    sample_idx = np.random.choice(len(X_val_raw), size=min(2000, len(X_val_raw)), replace=False)
    ...
    for lr in [0.004, 0.008, 0.015, 0.03]:
        m = XGBRegressor(n_estimators=300, max_depth=6, learning_rate=lr,
    ...
    depth_maes = {}
    for md in [4, 6, 8, 10]:
        m = XGBRegressor(n_estimators=300, max_depth=md, learning_rate=0.01,
                         tree_method='hist', random_state=42, verbosity=0)
        m.fit(X_train_raw, y_train)
        mae = mean_absolute_error(y_val_orig, np.expm1(m.predict(X_val_raw)))
        depth_maes[md] = mae
    ...
    axes[0].axvline(x=XGB_PARAMS['learning_rate'], color='red', linestyle='--',
    ...
    axes[1].axvline(x=XGB_PARAMS['max_depth'], color='red', linestyle='--',
