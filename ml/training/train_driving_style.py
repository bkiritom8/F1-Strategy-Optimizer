...
with aiplatform.start_run(run='driving-style-v3'):
    ...
    val_pred  = np.argmax(best_w * lgb.predict_proba(X_val)  + (1 - best_w) * xgb.predict_proba(X_val),  axis=1)
    test_pred = np.argmax(best_w * lgb.predict_proba(X_test) + (1 - best_w) * xgb.predict_proba(X_test), axis=1)
    ...
    aiplatform.log_metrics({'bias_mitigation_applied': int(f1_gap > 0.10), 'bias_position_f1_gap': float(f1_gap)})
    ...
    for lr in [0.003, 0.008, 0.02, 0.05]:
        m = LGBMClassifier(n_estimators=300, max_depth=8, learning_rate=lr,
                           random_state=42, n_jobs=-1, verbose=-1, class_weight='balanced')
        m.fit(X_train, y_train)
        f1 = f1_score(y_val, m.predict(X_val), average='macro')
        lr_f1s[lr] = f1
    ...
    axes[0].axvline(x=LGB_PARAMS['num_leaves'], color='red', linestyle='--',
    ...
    axes[1].axvline(x=LGB_PARAMS['learning_rate'], color='red', linestyle='--',
