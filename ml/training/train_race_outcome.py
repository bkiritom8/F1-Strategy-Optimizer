...
with aiplatform.start_run(run='race-outcome-v3'):
    ...
    cat_val_f1 = float(f1_score(y_val, cat_model.predict(X_val), average='macro', zero_division=0))
    lgb_val_f1 = float(f1_score(y_val, lgb_model.predict(X_val), average='macro', zero_division=0))
    ...
    aiplatform.log_metrics({
        ...
        'cat_val_f1': cat_val_f1, 'lgb_val_f1': lgb_val_f1,
    ...
    ax.bar(['CatBoost', 'LGB', 'Ensemble'], [cat_val_f1, lgb_val_f1, val_f1],
    ...
    for season in sorted(val['season'].unique()):
        ...
        f1   = float(f1_score(y_val_series[mask], val_pred_series[mask], average='macro', zero_division=0))
    ...
    mid_constructors = {'alpine', 'astonmartin', 'williams', 'haas', 'alfatauri', 'alfaromeo'}
    val['constructor_tier'] = val['constructorId'].apply(
        lambda x: 'top' if x in top_constructors else ('mid' if x in mid_constructors else 'back')
    )
    ...
    if f1_gap > 0.15:
        back_count       = (train['constructorId'].apply(
            lambda x: x not in top_constructors and x not in mid_constructors)).sum()
        top_count        = train['constructorId'].isin(top_constructors).sum()
        oversample_factor = max(1, top_count // max(back_count, 1))
        print(f'  Suggested oversample factor for back constructors: {oversample_factor}x')
        ...
        aiplatform.log_metrics({'bias_mitigation_applied': 0, 'bias_constructor_f1_gap': float(f1_gap)})
    ...
    depth_f1s  = {}
    for md in [4, 6, 8, 10]:
        m = LGBMClassifier(n_estimators=300, max_depth=md, num_leaves=31,
    ...
    axes[0].axvline(x=LGB_PARAMS['max_depth'], color='red', linestyle='--',
