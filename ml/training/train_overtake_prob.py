...
with aiplatform.start_run(run='overtake-prob-v2'):
    ...
    ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=['No Overtake', 'Overtake']).plot(
    ...
    # Slice by DRS zone
    for label, mask in [('in_drs', val['in_drs_zone']==1), ('not_in_drs', val['in_drs_zone']==0)]:
        if mask.sum() < 10:
            continue
        f1 = f1_score(y_val_series[mask], val_pred_series[mask], zero_division=0)
    ...
    axes[0].axvline(x=RF_PARAMS['max_depth'], color='red', linestyle='--',
    ...
    axes[1].axvline(x=RF_PARAMS['min_samples_leaf'], color='red', linestyle='--',
...
for feat, imp in sorted(zip(features, rf_inner.feature_importances_), key=lambda x: -x[1])[:15]:
    print(f'  {feat}: {imp:.4f}')
