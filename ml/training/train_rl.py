...
EVAL_RACE_IDS = [f"2025_{r}" for r in range(1, 13)]
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--skip-registry", action="store_true", help="Skip model registry push"
    )
    return p.parse_args()
...
def evaluate_bias_slices(
    ...
    for label, race_ids in [
        ...
        try:
            ...
            logger.info(
                "  Circuit slice %-8s → avg P%.1f  reward=%.1f",
                label,
                avg_pos,
                res["avg_reward"],
            )
    ...
    if circuit_results:
        ...
        if max_dev > 3.0:
            worst = max(
                circuit_results, key=lambda k: abs(circuit_results[k] - mean_pos)
            )
            logger.warning(
            ...
            logger.info(
                "  No circuit bias detected (max deviation=%.1f positions)", max_dev
            )
    ...
    for label, spos in [("front_p1", 1), ("mid_p10", 10), ("back_p18", 18)]:
        try:
            ...
            logger.info(
                "  Start pos %-10s → avg P%.1f  gain=%.1f",
                label,
                res["avg_position"],
                gain,
            )
    ...
    if not smoke:
        ...
        for season, race_ids in SEASON_SLICES.items():
            try:
                ...
                logger.info(
                    "  Season slice %s → avg P%.1f", season, res["avg_position"]
                )
            except Exception as exc:
...
def analyze_policy_sensitivity(
    ...
    try:
        ...
        ax.set_title(
            f"RL Policy Sensitivity — Feature Perturbation Analysis\n"
...
def _plot_bias_results(bias_metrics: dict[str, float]) -> None:
    ...
    try:
        ...
        values = [
            bias_metrics.get(f"bias_circuit_{l}_avg_position", None) for l in labels
        ]
        ...
        axes[0].set_title(
            "RL Policy Bias — Avg Position by Circuit Type\n(lower = better)"
        )
        ...
        if valid_sp:
            ...
            axes[1].set_title(
                "RL Policy — Position Gain by Starting Position\n(positive = improved)"
            )
...
def _plot_position_distribution(
    positions: list[int], title: str, filename: str
) -> None:
    ...
    try:
        ...
        ax.axvline(
            ...
            color="red",
            linestyle="--",
            label=f"mean P{np.mean(positions):.1f}",
        )
...
def compare_with_baseline(
    ...
    if not os.path.exists(baseline_path):
        logger.info(
            "No baseline model found at %s — accepting new model", baseline_path
        )
        return True
    ...
    try:
        ...
        logger.info(
            "  Baseline avg P%.1f  |  New model avg P%.1f  |  Δ=%.1f positions",
            baseline_avg,
            new_avg_position,
            improvement,
        )
...
def main() -> None:
    ...
    if args.smoke_test:
        ...
        logger.info(
            "=== SMOKE TEST MODE: %dk steps, %d envs ===",
            args.timesteps // 1000,
            args.n_envs,
        )
    else:
    ...
    logger.info(
        "Post-training evaluation (%d episodes)…", 5 if not args.smoke_test else 3
    )
    try:
        n_eps = 3 if args.smoke_test else 5
        eval_res = _eval_episodes(
            agent,
            eval_ids[:3] if args.smoke_test else eval_ids,
            adapters,
            args.driver_id,
            args.start_pos,
            n_eps,
        ...
        logger.info(
            "Post-training eval — avg P%.1f  reward=%.1f  pit_stops=%.1f",
            avg_pos,
            eval_res["avg_reward"],
            eval_res["avg_pit_stops"],
        )
        for i, (pos, rew) in enumerate(
            zip(eval_res["positions"], eval_res["rewards"]), 1
        ):
    ...
    if _VERTEX_AVAILABLE and _run_ctx is not None:
        try:
            ...
            sens_subset = {
                k: v for k, v in sensitivity_metrics.items() if isinstance(v, float)
            }
            if sens_subset:
    ...
    if bias_metrics:
        ...
        logger.info(
            "  Bias flag    : %s (circuit deviation=%.1f positions)",
