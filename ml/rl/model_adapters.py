...
_TELEMETRY_EST: dict[str, dict[str, float]] = {
    "PUSH": {
        "mean_throttle": 74.0, "std_throttle": 16.0,
        "mean_brake":    24.0, "std_brake":    10.0,
        "mean_speed":   232.0, "max_speed":    312.0,
        "SpeedI1":      188.0, "SpeedI2":      212.0,
        "SpeedFL":      262.0, "SpeedST":      292.0,
        "brake_roll3":   24.0, "throttle_roll3": 74.0,
    },
    "BALANCED": {
        "mean_throttle": 61.0, "std_throttle": 13.0,
        "mean_brake":    18.0, "std_brake":     8.0,
        "mean_speed":   220.0, "max_speed":    305.0,
        "SpeedI1":      177.0, "SpeedI2":      201.0,
        "SpeedFL":      251.0, "SpeedST":      281.0,
        "brake_roll3":   18.0, "throttle_roll3": 61.0,
    },
    "NEUTRAL": {
        "mean_throttle": 50.0, "std_throttle": 10.0,
        "mean_brake":    14.0, "std_brake":     6.0,
        "mean_speed":   209.0, "max_speed":    298.0,
        "SpeedI1":      166.0, "SpeedI2":      191.0,
        "SpeedFL":      241.0, "SpeedST":      271.0,
        "brake_roll3":   14.0, "throttle_roll3": 50.0,
    },
}

OPTIMAL_STINT: dict[str, int] = {
    "SOFT": 20, "MEDIUM": 30, "HARD": 45, "INTERMEDIATE": 25, "WET": 20,
}
...
class _BaseAdapter:
    ...
    def __init__(self, path: Optional[str], project: str = "f1optimizer") -> None:
        ...
        if path:
            try:
                ...
                logger.warning("%s: load failed (%s) — fallback active", self.model_name, exc)
    ...
    def _align(self, row: dict) -> pd.DataFrame:
        ...
        return pd.DataFrame([{f: row.get(f, 0.0) for f in features}])[features].fillna(0.0)

    def _align_batch(self, rows: list[dict]) -> pd.DataFrame:
        """Align a list of state dicts into a single feature DataFrame."""
        features = self._bundle["features"]
        return pd.DataFrame(
            [{f: r.get(f, 0.0) for f in features} for r in rows]
        )[features].fillna(0.0)
...
class SafetyCarAdapter(_BaseAdapter):
    ...
    def predict_pit(self, state: dict) -> float:
        ...
        try:
            ...
            proba   = w * pit_lgb.predict_proba(X)[:, 1] + (1 - w) * pit_xgb.predict_proba(X)[:, 1]
...
class PitWindowAdapter(_BaseAdapter):
    ...
    _OPTIMAL: dict[str, int] = {
        "SOFT": 20, "MEDIUM": 30, "HARD": 45, "SUPERSOFT": 18,
        "ULTRASOFT": 15, "HYPERSOFT": 12, "INTERMEDIATE": 25, "WET": 20,
    }
        ...
        try:
            ...
            row = {
                ...
                **cflags,
                ...
                "tyre_cubed":          tyre_cu,
                "tyre_per_stint":      tire_age / (stint + 1),
                "stint_progress":      min(1.0, tire_age / max(self._OPTIMAL.get(compound, 30), 1)),
                "fuel_load_pct":       fuel_pct,
                "laps_remaining":      laps_rem,
                "LapNumber":           lap,
                "lap_progress":        lap / total,
                ...
                "laps_until_optimal":  max(0, self._OPTIMAL.get(compound, 30) - tire_age),
                "mean_rpm":            11500.0,
                "max_rpm":             13000.0,
                "mean_gear":           6.5,
                "drs_usage_pct":       0.25,
...
def _heuristic_overtake_prob(state: dict) -> float:
    ...
    if gap <= 0.5:  return 0.35
    if gap <= 1.0:  return 0.20
    if gap <= 2.0:  return 0.08
    return 0.02
...
class RaceOutcomeAdapter(_BaseAdapter):
    ...
    def _predict_tier_proba(self, state: dict) -> dict[str, float]:
        ...
        row = {
            ...
            "constructor_rolling_avg_finish":    state.get("constructor_rolling_avg_finish", 10.0),
            ...
            "constructor_rolling_podiums":       state.get("constructor_rolling_podiums", 0.0),
            "driver_rolling_points_finishes":    state.get("driver_rolling_points_finishes", 0.3),
