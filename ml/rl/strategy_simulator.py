...
class StrategySimulator:
    ...
    def simulate_with_strategy(
        ...
        """Run the user's exact pit strategy through the full race simulator.

        Args:
            race_id:           e.g. "2026_1"
            user_driver_id:    e.g. "max_verstappen"
            strategy:          [(pit_lap, new_compound), ...] forced pit stops
            start_position:    Grid slot (1–20)
            start_compound:    Tire the driver starts on
            driver_profile:    Optional skill overrides
            rivals:            Rival driver IDs (None = default grid)
            n_stochastic_runs: Episodes used to estimate win/podium probability

        Returns:
            dict with keys: predicted_final_position, predicted_total_time_s,
        ...
        user_laps = result.lap_data.get(user_driver_id, [])
        lap_times_s = [round(lr.lap_time_ms / 1000.0, 3) for lr in user_laps]
        total_time_s = round(sum(lr.lap_time_ms for lr in user_laps) / 1000.0, 3)

        # Stochastic runs for win/podium probability
        position_counts = [0] * 21
        for seed in range(n_stochastic_runs):
            r = self._run_race(
                race_id,
        ...
        win_prob = round(position_counts[1] / max(n_stochastic_runs, 1), 4)
        podium_prob = round(sum(position_counts[1:4]) / max(n_stochastic_runs, 1), 4)

        return {
            "predicted_final_position": result.user_final_position,
